...
function StatCard({ label, value, colorClass, subtext }) {
  return (
    <div className={`bg-slate-800 rounded-xl border border-slate-700 shadow-sm px-5 py-4 flex flex-col gap-1 ${colorClass}`}>
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-900">
      {/* Top nav */}
      <header className="bg-slate-800 border-b border-slate-700 px-6 py-3 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          <span className="text-2xl select-none">🦞</span>
          <div>
            <span className="font-bold text-slate-100 text-lg leading-none">Lobster K8s Copilot</span>
            <p className="text-xs text-slate-500 leading-none mt-0.5">Kubernetes AI Diagnostics</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {clusterStatus ? (
            ...
            <span className="text-xs text-slate-500">No cluster status</span>
          )}
          <button
            onClick={handleRefresh}
            disabled={loading || refreshing}
            className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-indigo-400 px-3 py-1.5 rounded-lg border border-slate-700 hover:border-indigo-500 hover:bg-slate-700 transition-all disabled:opacity-40"
          >
